...
module.exports = {
  ...
  async create(req, res, next) {
    ...
    if (files.length === 0) {
      ...
      try {
        ...
        const data = await Product.create(product, id_user); // Agregar el parámetro id_user al llamar a Product.create()
        product.id = data.id;
        ...
        const start = async () => {
          await asyncForEach(files, async (file) => {
            ...
            await Product.update(product, id_user);
            inserts = inserts + 1;
  ...
  async update(req, res, next) {
    ...
    if (files.length === 0) {
      return res.status(501).json({
        message: "Error al actualizar el producto no tiene imagen",
        success: false,
      });
    } else if (files.length > 0 && files.length <= 3) {
      try {
        ...
        const data = await Product.update(product, id_user, id_product); // Agregar los parámetros id_user e id_product al llamar a Product.update()
        ...
        const start = async () => {
          await asyncForEach(files, async (file) => {
            ...
            await Product.update(product, id_user, id_product); // Agregar los parámetros id_user e id_product al llamar a Product.update()
            inserts = inserts + 1;
